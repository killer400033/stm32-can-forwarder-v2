...
// Unpack signals from TEMPERATURES
int8_t Unpack_TEMPERATURES(TEMPERATURES_t* _m, const uint8_t* _d, uint8_t len) {
	if (len < 8u) return STATUS_ERROR;

	// Extracting OilTemp0
	_m->OilTemp0 = (double)UNPACK_SCALE_OFFSET_TEMPERATURES_OILTEMP0(((_d[0] & 0xffu) << 8u) | ((_d[1] & 0xffu) >> 0u));
	// Extracting OilTemp1
	_m->OilTemp1 = (double)UNPACK_SCALE_OFFSET_TEMPERATURES_OILTEMP1(((_d[2] & 0xffu) << 8u) | ((_d[3] & 0xffu) >> 0u));
	// Extracting CoolantTemp0
	_m->CoolantTemp0 = (double)UNPACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP0(((_d[4] & 0xffu) << 8u) | ((_d[5] & 0xffu) >> 0u));
	// Extracting CoolantTemp1
	_m->CoolantTemp1 = (double)UNPACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP1(((_d[6] & 0xffu) << 8u) | ((_d[7] & 0xffu) >> 0u));

	return STATUS_OK;
}

// Unpack signals from POTENTIOMETERS
int8_t Unpack_POTENTIOMETERS(POTENTIOMETERS_t* _m, const uint8_t* _d, uint8_t len) {
	if (len < 4u) return STATUS_ERROR;

	// Extracting Potentiometer0
	_m->Potentiometer0 = (double)UNPACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER0(((_d[0] & 0xffu) << 8u) | ((_d[1] & 0xffu) >> 0u));
	// Extracting Potentiometer1
	_m->Potentiometer1 = (double)UNPACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER1(((_d[2] & 0xffu) << 8u) | ((_d[3] & 0xffu) >> 0u));

	return STATUS_OK;
}

// Unpack signals from STRAIN_GAUGE_LINKAGES
int8_t Unpack_STRAIN_GAUGE_LINKAGES(STRAIN_GAUGE_LINKAGES_t* _m, const uint8_t* _d, uint8_t len) {
	if (len < 8u) return STATUS_ERROR;

	// Extracting StrainLinkage0
	_m->StrainLinkage0 = ((_d[0] & 0xffu) << 8u) | ((_d[1] & 0xffu) >> 0u);
	// Extracting StrainLinkage1
	_m->StrainLinkage1 = ((_d[2] & 0xffu) << 8u) | ((_d[3] & 0xffu) >> 0u);
	// Extracting StrainLinkage2
	_m->StrainLinkage2 = ((_d[4] & 0xffu) << 8u) | ((_d[5] & 0xffu) >> 0u);
	// Extracting StrainLinkage3
	_m->StrainLinkage3 = ((_d[6] & 0xffu) << 8u) | ((_d[7] & 0xffu) >> 0u);

	return STATUS_OK;
}

// Unpack signals from STRAIN_GAUGE_STEERING
int8_t Unpack_STRAIN_GAUGE_STEERING(STRAIN_GAUGE_STEERING_t* _m, const uint8_t* _d, uint8_t len) {
	if (len < 4u) return STATUS_ERROR;

	// Extracting StrainSteering0
	_m->StrainSteering0 = ((_d[0] & 0xffu) << 8u) | ((_d[1] & 0xffu) >> 0u);
	// Extracting StrainSteering1
	_m->StrainSteering1 = ((_d[2] & 0xffu) << 8u) | ((_d[3] & 0xffu) >> 0u);
...
// Pack signals from TEMPERATURES
int8_t Pack_TEMPERATURES(const TEMPERATURES_t* _m, uint8_t* _d, uint8_t len) {
	if (len < 8u) return STATUS_ERROR;

	for (uint8_t i = 0u; i < 8u; _d[i++] = 0u);

	_d[0] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_OILTEMP0(_m->OilTemp0)) >> 8u) & 0xffu);
	_d[1] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_OILTEMP0(_m->OilTemp0)) << 0u) & 0xffu);
	_d[2] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_OILTEMP1(_m->OilTemp1)) >> 8u) & 0xffu);
	_d[3] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_OILTEMP1(_m->OilTemp1)) << 0u) & 0xffu);
	_d[4] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP0(_m->CoolantTemp0)) >> 8u) & 0xffu);
	_d[5] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP0(_m->CoolantTemp0)) << 0u) & 0xffu);
	_d[6] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP1(_m->CoolantTemp1)) >> 8u) & 0xffu);
	_d[7] |= (( ((uint16_t)PACK_SCALE_OFFSET_TEMPERATURES_COOLANTTEMP1(_m->CoolantTemp1)) << 0u) & 0xffu);

	return STATUS_OK;
}

// Pack signals from POTENTIOMETERS
int8_t Pack_POTENTIOMETERS(const POTENTIOMETERS_t* _m, uint8_t* _d, uint8_t len) {
	if (len < 4u) return STATUS_ERROR;

	for (uint8_t i = 0u; i < 4u; _d[i++] = 0u);

	_d[0] |= (( ((uint16_t)PACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER0(_m->Potentiometer0)) >> 8u) & 0xffu);
	_d[1] |= (( ((uint16_t)PACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER0(_m->Potentiometer0)) << 0u) & 0xffu);
	_d[2] |= (( ((uint16_t)PACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER1(_m->Potentiometer1)) >> 8u) & 0xffu);
	_d[3] |= (( ((uint16_t)PACK_SCALE_OFFSET_POTENTIOMETERS_POTENTIOMETER1(_m->Potentiometer1)) << 0u) & 0xffu);

	return STATUS_OK;
}

// Pack signals from STRAIN_GAUGE_LINKAGES
int8_t Pack_STRAIN_GAUGE_LINKAGES(const STRAIN_GAUGE_LINKAGES_t* _m, uint8_t* _d, uint8_t len) {
	if (len < 8u) return STATUS_ERROR;

	for (uint8_t i = 0u; i < 8u; _d[i++] = 0u);

	_d[0] |= (( ((uint16_t)(_m->StrainLinkage0)) >> 8u) & 0xffu);
	_d[1] |= (( ((uint16_t)(_m->StrainLinkage0)) << 0u) & 0xffu);
	_d[2] |= (( ((uint16_t)(_m->StrainLinkage1)) >> 8u) & 0xffu);
	_d[3] |= (( ((uint16_t)(_m->StrainLinkage1)) << 0u) & 0xffu);
	_d[4] |= (( ((uint16_t)(_m->StrainLinkage2)) >> 8u) & 0xffu);
	_d[5] |= (( ((uint16_t)(_m->StrainLinkage2)) << 0u) & 0xffu);
	_d[6] |= (( ((uint16_t)(_m->StrainLinkage3)) >> 8u) & 0xffu);
	_d[7] |= (( ((uint16_t)(_m->StrainLinkage3)) << 0u) & 0xffu);

	return STATUS_OK;
}

// Pack signals from STRAIN_GAUGE_STEERING
int8_t Pack_STRAIN_GAUGE_STEERING(const STRAIN_GAUGE_STEERING_t* _m, uint8_t* _d, uint8_t len) {
	if (len < 4u) return STATUS_ERROR;

	for (uint8_t i = 0u; i < 4u; _d[i++] = 0u);

	_d[0] |= (( ((uint16_t)(_m->StrainSteering0)) >> 8u) & 0xffu);
	_d[1] |= (( ((uint16_t)(_m->StrainSteering0)) << 0u) & 0xffu);
	_d[2] |= (( ((uint16_t)(_m->StrainSteering1)) >> 8u) & 0xffu);
	_d[3] |= (( ((uint16_t)(_m->StrainSteering1)) << 0u) & 0xffu);
